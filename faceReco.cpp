...
void saySomething(int nWho)
{
	...
	if (nbSpeak[nWho]==0)
	{
		...
		else
		{
			...
			if (nWho==USER2)
			{
				...
				fprintf(f,"Je te souhaite une excellente fin de journÃƒÂ©e\n");							
			}
			if (nWho==USER1)
			{
	...
	if (nbSpeak[nWho]==15)
	{
		...
		if (f==NULL)
		{
			cout<<"(E) impossible d'ÃƒÂ©crire le fichier\n";
			return;
		}
		else
		{
			...
			if (nWho==USER1)
			{
				...
				fprintf(f,"%s, que dirais tu de prendre un petit cafÃƒÂ© ?.\n",people[nWho].c_str()); 		
			}
			if (nWho==USER3)
			{
				// ...
			...
			if (nWho==USER1) system("espeak -f speak.txt -vfr+f5 -s130 2>/dev/null");
			if (nWho==USER2) system("espeak -f speak.txt  -s130  2>/dev/null");
		}
	}
